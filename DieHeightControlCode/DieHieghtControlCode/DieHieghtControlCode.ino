...
void callback(char* topic, byte* payload, unsigned int length) {
    ...
    const float computationalThreshold = 0.1;
...
void operateRelay(uint8_t relayPin, float targetHeight, float threshold, bool isUpward) {
    ...
    while (true) {
        ...
        if (isUpward) {
            ...
            // If we're very close but overshooting, stop immediately
            if (difference < 0.02) {
                delay(520);  // Small delay to ensure relay deactivation timing
                break;
            }
        } else {
            if (-difference <= threshold) {
            ...
            // If we're very close but undershooting, stop immediately
            if (difference > -0.02) {
                delay(520);  // Small delay to ensure relay deactivation timing
                break;
            }
        ...
        delay(100);  // Small delay to prevent rapid iterations
    }
